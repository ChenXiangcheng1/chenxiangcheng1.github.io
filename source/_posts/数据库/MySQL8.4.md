...
# psql -U nemesis -c "\password postgres"  # 设置数据库用户postgres的密码
...
# psql -U nemesis -c "\password postgres"
exit
sudo systemctl enable postgresql
sudo systemctl status postgresql
...
| /var/lib/postgres/data/postgresql.conf | 配置文件                                                     |
...
# trust(不验证)、md5(单次哈希)、scram-sha-256(推荐)、peer(验证Linux系统用户名是否存在该同名数据库用户)

# "local" is for Unix domain socket connections only  # UNIX Socket
local   all             all                                     peer
# IPv4 local connections:  # TCP/IP
host    all             postgres,nemesis             0.0.0.0/0            scram-sha-256
# IPv6 local connections:
host    all             postgres,nemesis             ::1/128                 scram-sha-256
...
| \l+                                                          | 查看所有数据库                                               |
| \d                                                           | 查看表、视图、序列                                           |
| \dt <schema_name>.*                                          | 查看目标schema下的所有表                                     |
| \conninfo                                                    |                                                              |
| \q                                                           |                                                              |
|                                                              |                                                              |
| **以下需要使用postgres用户**                                 |                                                              |
...
Schema
pg_catalog 系统表relation(系统目录catalog)
...
系统表
pg_database 所有数据库的定义
pg_class 所有表
pg_user
pg_roles
pg_namespace 所有Schema
pg_tablespace 表空间(控制对象存储位置)的定义
pg_proc 所有函数和存储过程的定义
pg_locks
...
> 我是更推荐Navicat Premium(推荐)、DBeaver、DataGrip(重)

使用Flask+ReactJS写的WebUI
...
```postgresql
-- MySQL DDL建库参数直接跟在语句后传递， postgresql通过with块传递参数
```
