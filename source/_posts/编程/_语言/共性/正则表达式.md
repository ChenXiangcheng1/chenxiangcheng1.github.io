...
|                 |                                                              |                                                              |        |
| .               | 匹配除换行符(\r \n)以外的任意字符                            |                                                              |        |
| [\s\S]          | 匹配任意字符包括换行空白字符，和re.DOTALL的.效果一样         |                                                              |        |
...
| 限定符 | 量词默认是贪婪模式<br />例如<h1>.*</h1>，`.*` 会贪婪地“吞掉”所有字符，直到它无法再匹配为止，然后引擎才会回溯，找到最后一个 `</h1>` 来满足整个表达式<br />`.*?` ?是非贪婪模式，尽可能少匹配 |
| ------ | ------------------------------------------------------------ |
| *      | 重复零次或更多次                                             |
| +      | 重复一次或更多次                                             |
| ?      | 重复零次或一次                                               |
...
re.Dotall 也有人叫单行模式，将多行文本看作一个单行来处理，.可以匹配换行符
